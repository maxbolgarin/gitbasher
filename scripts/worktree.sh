...
function worktree_script {
    ...
    if [ -n "$help" ]; then
        ...
        print_help_row $PAD "<empty>" ""                  "Show interactive worktree menu"
        print_help_row $PAD "list"    "l, ls"             "List all worktrees"
        print_help_row $PAD "add"     "a, new, n, c"      "Create a worktree with a new branch from current HEAD"
        print_help_row $PAD "addd"    "ad, nd"            "Fetch + create worktree with new branch from $main_branch"
        print_help_row $PAD "addb"    "ab, from, b"       "Create a worktree from an existing local branch"
        print_help_row $PAD "addr"    "ar, remote, r"     "Fetch + create worktree tracking a remote branch"
        print_help_row $PAD "manage"  "m"                 "Pick a worktree, then move / lock / unlock / delete"
        print_help_row $PAD "move"    "mv"                "Move a worktree to a new path"
        print_help_row $PAD "lock"    ""                  "Lock a worktree (with optional reason)"
        print_help_row $PAD "unlock"  "ul"                "Unlock a worktree"
        print_help_row $PAD "remove"  "rm, del, d"        "Remove a worktree (interactive picker)"
        print_help_row $PAD "prune"   "pr, p"             "Remove stale worktree records"
        print_help_row $PAD "path"    "sw"                "Print the path to a chosen worktree"
        print_help_row $PAD "goto"    "go, g, cd, switch" "Open a subshell inside a chosen worktree"
