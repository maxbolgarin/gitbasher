...
### 1. **CRITICAL: `tail -r` is not portable to Linux**
**Location:** `scripts/common.sh:870`, `scripts/merge.sh:260`
**Severity:** Critical
**Impact:** Code will fail on Linux systems
...
The `tail -r` option is BSD/macOS specific and does not exist on Linux. This causes the script to fail on Linux systems with "tail: invalid option -- 'r'".

**Fix:**
Use `tac` (reverse of cat) for cross-platform compatibility:
```bash
# Option 1: Use tac (available on Linux)
conflicts="$(echo "$switch_output" | tac | tail -n +3 | tac | tail -n +2)"

# Option 2: Detect OS and use appropriate command
...
**Locations to fix:**
- `scripts/common.sh:870`
- `scripts/merge.sh:260`
